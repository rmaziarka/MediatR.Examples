namespace KnightFrank.Antares.Domain.Contact.Commands
{
    ...
    public class CreateContactCommandValidator : AbstractValidator<CreateContactCommand>
    {
        public CreateContactCommandValidator()
        {
			...
			this.RuleFor(x => x.FirstName).Length(1, 128);
			...
			this.RuleFor(x => x.MailingInformalSalutation).Length(1, 128);
			this.RuleFor(x => x.MailingPersonalSalutation).Length(1, 128);
			this.RuleFor(x => x.MailingEnvelopeSalutation).Length(1, 128);

			this.RuleFor(x => x.EventInviteSalutation).Length(1, 128);
			this.RuleFor(x => x.EventSemiformalSalutation).Length(1, 128);
